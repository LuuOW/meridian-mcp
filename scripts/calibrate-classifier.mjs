...
const TASK = 'design a public benchmark for tool-routing failures in LLM agents'
...
// Strip labels before classification (production callers don't have them).
const skillsForClassify = PANEL.map(({ __expectedClass, __expectedSystem, __relevant, ...skill }) => skill)

// ── Run ────────────────────────────────────────────────────────────
const ranked = orbitalClassify(skillsForClassify, TASK)
